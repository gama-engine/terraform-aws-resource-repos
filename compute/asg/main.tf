...
resource "aws_autoscaling_group" "main" {
  ...
  vpc_zone_identifier = [ ] # Private Subnet IDs
  target_group_arns = [] # Target Group ARN(s)
  health_check_type = # EC2(If you're not using a Load Balancer) | ELB(If you're using an Application Load Balancer)
...
resource "aws_autoscaling_policy" "target_tracking" {
  ...
  target_tracking_configuration {
    ...
    target_value = # Target Metric Value, Keep average CPU around X%.
...
resource "aws_autoscaling_policy" "scale_out" {
  ...
  scaling_adjustment = # Number of Instances to add
  cooldown = # Seconds
...
resource "aws_autoscaling_policy" "scale_in" {
  ...
  autoscaling_group_name = # Auto Scaling Group Name
  adjustment_type = "ChangeInCapacity"
  scaling_adjustment = # Negative Number to remove
  cooldown = # Seconds
   autoscaling_group_name =
}
...
resource "aws_cloudwatch_metric_alarm" "high_cpu" {
  ...
  evaluation_periods = # Number of Evaluation Periods in Sec, Number of measurements to evaluate before changing the alarm state.
  metric_name = "CPUUtilization"
  namespace = "AWS/EC2"
  period = # Seconds, Measure CPU every X minute
  statistic = "Average"
  threshold = # CPU Percentage
  alarm_actions = []  # Scale Out Policy ARN
  dimensions = {
...
resource "aws_cloudwatch_metric_alarm" "low_cpu" {
  ...
  statistic = "Average"
  threshold = # CPU Percentage
  alarm_actions = [] # Scale In Policy ARN
  dimensions = {
    AutoScalingGroupName = # Auto Scaling Group Name
  }
}
